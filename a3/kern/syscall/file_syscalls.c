...
sys___getcwd(userptr_t buf, size_t buflen, int *retval)
{
    ...
    mk_useruio(&iov, &uuio, buf, buflen, 0, UIO_READ);

    result = vfs_getcwd(&uuio);

    if (result) {
        return result;
    }

    *retval = buflen - uuio.uio_resid;
 ...
 * sys_getdirentry
 *
 * Read next filename from a directory.
 */
int
sys_getdirentry(int fd, userptr_t buf, size_t buflen, int *retval)
{
    ...
    mk_useruio(&iov, &uuio, buf, buflen, f->offset, UIO_READ);

    result = VOP_GETDIRENTRY(f->v, &uuio);
